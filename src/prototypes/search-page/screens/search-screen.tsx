...
/**
 * SearchScreen — the main "Search" view.
 *
 * Wires together:
 ...
 *
 * All state (query, filters, sort, source, sheet open, sort dropdown open,
 * topbar collapsed) lives here; child components are presentational.
 *
 * Behaviour ported 1:1 from script.js. Minor deviation: picking a recent
 * search submits via the debounced hook (500ms) instead of firing
 * immediately — the original `doSearch()` call had no debounce there.
 */
...
export function SearchScreen() {
  ...
  // Add to recents 500ms after the user stops typing (matches original).
  useEffect(() => {
    if (!query) return;
  ...
  return (
    <section className={styles.view} aria-label="Search">
